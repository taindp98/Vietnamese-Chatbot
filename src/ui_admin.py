...
class Main:
        ...
        def check(*args):
            ...
            if key == "type_edu":
                for i in self.data_cons_dt["tuition_type_edu"].keys():
                    if i in variable.get():
                        self.result_dt.update(
                            {"tuition": self.data_cons_dt["tuition_type_edu"][i]}
                        )
            ...
            if key == "major_code":
                for i in self.data_cons_dt["career_major_code"].keys():
                    if i in variable.get():
                        self.result_dt.update(
                            {"career": self.data_cons_dt["career_major_code"][i]}
                        )
            ...
            if key == "case":
                for i in self.data_cons_dt["object_case"].keys():
                    if i in variable.get():
                        self.result_dt.update(
                            {"object": self.data_cons_dt["object_case"][i]}
                        )
                        self.result_dt.update(
                            {"register": self.data_cons_dt["register_case"][i]}
                        )
            ...
            if key == "subject_group":
                ...
                if "a00" in variable.get():
                    self.result_dt.update(
                        {key.lower(): [variable.get().replace(", ", "', '")]}
                    )
                    ...
                    self.result_dt.update(
                        {"subject": ["toán", "vật lý", "vẽ hình họa mỹ thuật"]}
                    )
                    ...
                    self.result_dt.update(
                        {"subject": ["toán", "ngữ văn", "vẽ hình họa mỹ thuật"]}
                    )
    ...
    def main(self):
        ...
        title1 = tk.Label(
            master, text="TƯ VẤN TUYỂN SINH ĐẠI HỌC", background="#CEE3F6"
        )
        title1.config(font=("Courier", 30))
        title1.place(x=730, y=110)

        # Create Combobox
        combox1 = Main.Combobox(
            self, "Mã ngành", {"major_code": sorted(self.data_dt["major_code"])}, 100
        )
        combox2 = Main.Combobox(
            self,
            "Chuyên ngành",
            {"major_name": sorted(self.data_dt["major_name"])},
            160,
        )
        combox3 = Main.Combobox(
            self,
            "Hình thức đào tạo",
            {"type_edu": sorted(self.data_dt["type_edu"])},
            220,
        )
        combox4 = Main.Combobox(
            self, "Năm", {"year": sorted(self.data_dt["year"] + ["2021", "2022"])}, 280
        )
        combox5 = Main.Combobox(
            self, "Khối", {"subject_group": sorted(self.data_dt["subject_group"])}, 340
        )
        combox6 = Main.Combobox(
            self, "Phương thức tuyển sinh", {"case": sorted(self.data_dt["case"])}, 400
        )
        ...
        def gen_output():
            ...
            text.insert(
                END, str(self.result_dt).replace('["', "['").replace('"]', "']")
            )
        ...
        button = ttk.Button(master, text="Output Generate", command=gen_output).place(
            x=180, y=585
        )
        button = ttk.Button(master, text="Submit Document", command=submit).place(
            x=340, y=585
        )
...
# Execuse File
execuse = Main(
    "./data/entity_admin.json", "./data/LogoBK.png", "./data/dict_consist.json"
)
