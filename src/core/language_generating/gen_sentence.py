...
class ResponseGeneration:
    ...
    def run(self, agent_action, state_tracker, confirm_obj, is_greeting=False):
        ...
        if agent_intent == "inform":
            ...
            sentence = sentence_pattern.replace("*{}*".format(inform_slot), self.response_gallery['agent_response_object'][inform_slot])
            ...
            if len(agent_action['inform_slots'][inform_slot]) > 1:
                ...
                sentence = sentence_pattern.replace("*request_slot*", self.response_gallery['agent_response_object'][inform_slot])
            ...
            sentence = sentence_pattern.replace("*{}*".format(request_slot), self.response_gallery['agent_response_object'][request_slot])
            list_sentence.append(sentence)
            ...
            if agent_action['inform_slots']['major'] == "no_match_available":
                sentence_pattern = random.choice(self.response_gallery['match_found']['not_found'])
                sentence = sentence_pattern.replace("*found_slot*", self.response_gallery['agent_response_object'][inform_slot])
                list_sentence.append(sentence)
                    ...
                    if check_match:
                        if inform_slot != 'point':
                            response_match = "\n\nĐúng rồi! Thông tin {0} bạn cần là {1}".format(
                                self.response_gallery['agent_response_object'][inform_slot],value_match
                            )
                        else:
                ...
                if inform_slot != "major":
                    if len(list_unique_slot_in_record_match) == 1:
                        ...
                        if len(first_record) > 1:
                            ...
                            sentence = self.response_gallery['empty_slot'][0].replace(
                                "*request_slot*", self.response_gallery['agent_response_object'][inform_slot]
                            )
                    ...
                    # print('here')
                    sentence = random.choice(self.response_gallery['match_found']['found_major'])
                    list_sentence.append(sentence)
