from dqn.dialogue_config import usersim_default_key, FAIL, NO_OUTCOME, SUCCESS,UNSUITABLE, usersim_required_init_inform_keys, \
    no_query_keys
from dqn.utils import reward_function
import random, copy
...
class UserSimulator:
    ...
    def _response_to_match_found(self, agent_action):
        ...
        for key, value in self.goal['inform_slots'].items():
            ...
            if value != agent_informs.get(key, None):
                self.constraint_check = FAIL
                # print("match found failed by no match inform value")
    ...
    def _response_to_done(self):
        ...
        # print("done: input constraint_check: {0}".format(self.constraint_check))
