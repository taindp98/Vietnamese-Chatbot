...
class IntentRecognition:
    def __init__(self, root: str, ai_service=None):
        ...
        self.dict_question_signal = load_pattern(
            os.path.join(root, "question_signal.json")
        )
        self.dict_random_intent = load_pattern(os.path.join(root, "random_intent.json"))
        self.dict_rule_based_intent = load_pattern(
            os.path.join(root, "business_intent.json")
        )
    ...
    def _check_is_question(self, mess: str):
        for k in list(self.dict_question_signal.keys()):
            is_question = self._find_pattern(
                query=mess, gallery=self.dict_question_signal[k]
            )
            if is_question:
                return is_question
            else:
    ...
    def _classify_intent(self, mess: str, dict_intent: dict):
        for intent in dict_intent:
            is_valid = self._find_pattern(query=mess, gallery=dict_intent[intent])
            if is_valid:
                return intent, 1.0
            else:
                continue
    ...
    def ai_based_predict_intent(self, mess: str):
        pred = requests.post(self.ai_service, json={"message": mess}).json()
    ...
    def predict(self, mess: str):
        ...
        if is_question:
            ...
            (intent, probability) = self._classify_intent(
                mess=cleaned_mess, dict_intent=self.dict_rule_based_intent
            )
            if intent == self.default_intent and self.ai_service:
                ## need to AI-based
                (intent, probability) = self.ai_based_predict_intent(mess=cleaned_mess)
        else:
            ## 3. Classify the random intent
            (intent, probability) = self._classify_intent(
                mess=cleaned_mess, dict_intent=self.dict_random_intent
            )
...
if __name__ == "__main__":
    ...
    message = (
        "em muốn hỏi là mình đăng kí xét tuyển chương trình việt pháp như thế nào ạ"
    )
