        ...
        if isinstance(mess, str):
            if not mess.startswith("/"):
                ...
                if not catched_intent in self.ignore_intent:
                    ...
                    dict_entity_inform, confirm_obj = self.entity_recognizer.find(
                        catched_intent, cleaned_mess
                    )
                    ...
                    if last_agent_action["intent"] != "match_found":
                        ...
                        if len(list(last_agent_action["request_slots"].keys())) > 0:
                            user_inform_key = list(
                                last_agent_action["request_slots"].keys()
                            )[0]

                        elif len(list(last_agent_action["inform_slots"].keys())) > 0:
                            user_inform_key = list(
                                last_agent_action["inform_slots"].keys()
                            )[0]

                        if (
                            len(list(last_agent_action["request_slots"].keys())) > 0
                            or len(list(last_agent_action["inform_slots"].keys())) > 0
                        ):
                        ...
                        (
                            ...
                            confirm_obj,
                        ) = self.entity_recognizer.find(final_intent, cleaned_mess)
                        ...
                        user_action["intent"] = "inform"
                        user_action["inform_slots"] = {
                            other_key_avoid_crash: "anything"
                        }
                        user_action["request_slots"] = {}
                    ...
                    if last_agent_action["intent"] != "match_found":
                        ...
                        if catched_intent == "agree":
                            user_action["inform_slots"] = dict(
                                last_agent_action["inform_slots"].items()
                            )
                        else:
                            ## fix entity

                            dict_fix_entity, confirm_obj = self.entity_recognizer.find(
                                final_intent, cleaned_mess
                            )
                    ...
                    last_agent_action = state_tracker.history[-1]
                    if len(list(last_agent_action["request_slots"].keys())) > 0:
                        anything_key = list(last_agent_action["request_slots"].keys())[
                            0
                        ]
                    elif len(list(last_agent_action["inform_slots"].keys())) > 0:
